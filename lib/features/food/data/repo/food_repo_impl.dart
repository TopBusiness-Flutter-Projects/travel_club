...
class FoodRepoImpl {
  ...
  Future<Either<Failure, GetRestaurantModel>> getRestuarnt({required String id}) async {
    try {
      var response = await dio.get(
        EndPoints.getRestaurantUrl,
        queryParameters: {
        if (id != "0")  "restaurant_category_id=" : id
        }
      );
  ...
  }Future<Either<Failure, GetRestaurantDetailsModel>> getRestaurantDetails({required String id}) async {
    try {
      var response = await dio.get(
