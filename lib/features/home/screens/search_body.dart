                ...
                child: CustomTextField(
                  ...
                  onChanged: (value) {
                    EasyDebounce.debounce(
                        ...
                        () async =>
                            await cubit.getHomeFilterData(context: context)());
                  },
                ),
              ),
            ],
            ...
            children: [
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) => Padding(
                                  ...
                                  child: CustomLodgeContainer(
                                    lodgesModel: cubit.residenceFavouriteModel
                                            .data?[index] ??
                                        LodgeModel(),
                                  ),
                                ),
                ...
                Expanded(
                    child: state is LoadingHomeFilterDataState
                        ...
                        : cubit.transportationFavouriteModel.data?.isEmpty ??
                            ...
                            : ListView.builder(
                                itemCount: cubit.transportationFavouriteModel
                                        .data?.length ??
                                    0,
              ...
              if (cubit.selectedIndex == 2) ...[
                ...
                Expanded(
                  child: state is LoadingHomeFilterDataState
                      ? Center(child: CustomLoadingIndicator())
                      : cubit.getRestaurantModel?.data?.isEmpty ?? true
                          ? Center(
                              child: NoDataWidget(title: "no_data".tr()),
                            )
                          : ListView.separated(
                              shrinkWrap: true,
                              itemCount:
                                  cubit.getRestaurantModel?.data?.length ?? 0,
                              itemBuilder: (BuildContext context, int index) {
                                return BigContainerFood(
                                  resturantData:
                                      cubit.getRestaurantModel?.data?[index] ??
                                          ResturantData(),
                                );
                              },
                              separatorBuilder:
              ...
              if (cubit.selectedIndex == 3) ...[
                Expanded(
                  child: state is LoadingHomeFilterDataState
                      ? Center(child: CustomLoadingIndicator())
                      : cubit.getWaysModel.data?.isEmpty ?? false
                          ? Center(
                              child: NoDataWidget(title: "no_data".tr()),
                            )
                          : GridView.builder(
